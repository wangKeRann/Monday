...
def import_blood_routine(conn, df, record_mapping):
    ...
    for index, row in df.iterrows():
        try:
            ...
            # 定义血常规指标列名（移除BC_前缀）
            bc_columns = [
                'white_blood_cell', 'neutrophil_percentage', 'lymphocyte_percentage',
                'monocyte_percentage', 'eosinophil_percentage', 'basophil_percentage',
                'neutrophil_absolute', 'lymphocyte_absolute', 'monocyte_absolute',
                'eosinophil_absolute', 'basophil_absolute', 'red_blood_cell',
            ...
            for col in bc_columns:
                value = row.get(f'BC_{col}')  # 在获取数据时添加前缀
                if pd.isna(value):
...
def import_urine_routine(conn, df, record_mapping):
    ...
    for index, row in df.iterrows():
        try:
            ...
            # 定义尿常规指标列名（移除UC_前缀）
            uc_columns = [
                'sugar', 'bilirubin', 'ketone',
                'specific_gravity', 'ph', 'protein',
                'urobilinogen', 'nitrite', 'blood',
                'leukocyte_esterase', 'red_blood_cell_microscopy',
                'white_blood_cell_microscopy', 'pus_cell_microscopy',
                'epithelial_cell_microscopy', 'granular_cast_microscopy',
                'hyaline_cast_microscopy', 'mucus_thread_microscopy',
                'fungus_microscopy'
            ]
            
            # 按顺序获取数据
            for col in uc_columns:
                value = row.get(f'UC_{col}')  # 在获取数据时添加前缀
                if pd.isna(value):
    ...
    for index, row in df.iterrows():
        try:
            ...
            bio_columns = [
                ...
                'albumin_globulin_ratio', 'alt', 'alp',
                'ggt', 'ast', 'ldh',
                'ck', 'ck_mb', 'total_bile_acid',
                'fibronectin', 'haptoglobin', 'alpha1_acid_glycoprotein',
                'potassium', 'sodium', 'chloride',
                'calcium', 'urea', 'creatinine',
                'uric_acid', 'cystatin_c', 'blood_glucose',
            ...
            for col in bio_columns:
                value = row.get(f'BIO_{col}')  # 在获取数据时添加前缀
                if pd.isna(value):
...
def import_ultrasound(conn, df, record_mapping):
    ...
    VALUES (%s, %s, %s, %s, %s, %s)
    ON DUPLICATE KEY UPDATE
        liver_status = VALUES(liver_status),
        thyroid_status = VALUES(thyroid_status),
        prostate_status = VALUES(prostate_status),
        neck_vessel_status = VALUES(neck_vessel_status),
        bladder_ureter_status = VALUES(bladder_ureter_status)
    """
...
def import_liver_fibrosis(conn, df, record_mapping):
    ...
    VALUES (%s, %s, %s, %s, %s, %s)
    ON DUPLICATE KEY UPDATE
        fibrosis_index = VALUES(fibrosis_index),
        hyaluronic_acid = VALUES(hyaluronic_acid),
        type_iv_collagen = VALUES(type_iv_collagen),
        laminin = VALUES(laminin),
        procollagen_iii = VALUES(procollagen_iii)
    """
